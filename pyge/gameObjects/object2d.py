...
class Object2D:
		...
		if len(self.edges) > 0:
			for i in range(len(self.edges)):
				for j in range(len(self.edges[i])):
					glVertex2f(self.verticies[self.edges[i][j]][0], self.verticies[self.edges[i][j]][1])
	...
	def move(self, x_move:float = 0, y_move:float = 0):
		"""
	...
	def scale(self, x_scale:float = 0, y_scale:float = 0):
		"""
